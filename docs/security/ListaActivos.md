...
| AST-03 | Acceso a Postgres (`DB_PASSWORD`, red interna) | `docker-compose.yml:6`, `DATABASE_URL` | Infra/DevOps | C:Alto · I:Alto · D:Alto | **Crítico** | Contiene simultáneamente AST-04 a AST-09. No es un activo con valor propio — es el de mayor concentración: un solo compromiso expone todo lo demás a la vez. | Sin `ports:` publicado en compose (no alcanzable desde el host). **Desde 2026-07-15**: la app ya no se conecta con el usuario bootstrap (`app`, que Postgres obliga a mantener superusuario) sino con `app_runtime` (`backend/db-init/10-create-app-runtime-role.sh`), un rol sin `SUPERUSER`/`CREATEDB`/`CREATEROLE`/`REPLICATION` — verificado con `\du` y con un intento fallido de `CREATE ROLE` desde ese rol. Red segmentada (`backend-net`) separa `db`/`redis` de `frontend`. | 04 — backups cifrados y prueba de restauración siguen pendientes (requieren panel de Railway); acción manual pendiente: el volumen `pgdata` local ya existente necesita recrearse una vez para que `app_runtime` exista en él (ver `docs/security/Docker.md`) |
